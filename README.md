...
### 配置对话机器人
_节标题H1, H2自带单横线, H3不用横线, 紧接着的H4可用一级编号, 紧接着的H5可用二级编号, 紧接着的H6可用`backtick quotes`._
...
机器人给出开场白后会与用户进行多轮对话。一轮对话由一句用户输入和一句机器人回复组成。机器人收到用户输入后，会先进行意图识别。如果此时没有待处理的流程，并且也没有识别到用户意图，则回复兜底话术。如果此时有待处理流程，但是识别到的意图对应的流程不是待处理流程，那么机器人去执行识别到的意图所对应的对话流，否则执行待处理流程。然后机器人开始处理对话流中的节点，然后跳转至下一节点，不断循环，直到在dm或回复节点或填槽节点产生了response，即机器人回复，此时在下一待处理节点处停止，把response回复给用户，一轮交互完成。接着等待用户输入，开始下一轮的交互。
...
##### 1. 文字版对话系统服务端启动脚本：run_server_text.sh _确认有没有带run_
...
| -------- | -------- | ------------------------------------------------------------ |
| userid   | str      | 对话唯一标识                                                 |
| inaction | int      | 指令标志。初始化对话时设为8 , 对话过程中设为最近一次从服务端接收到的【相应中的outaction: 修改】。8为初始化，9为正常交互。 |
...
目前同“文字版对话机器人API”。是不是把“电话版对话机器人API”写前面，“文字版对话机器人API”写后面。
...
| -------- | -------- | ------------------------------------------------------------ |
| userid   | str      | 对话唯一标识                                                 |
| inaction | int      | 指令标志。初始化对话时设为8 , 对话过程中设为最近一次从服务端接收到的【相应中的outaction: 修改】。8为初始化，9为正常交互，11为呼叫转移。 |
| inparams | json     | 客户端给服务端发送的参数列表                                 |
