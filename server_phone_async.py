...
class MainHandler(tornado.web.RequestHandler):
	...
	async def post(self):
			...
			# 有待完成指令
				...
				if user_input:
					self.db.add_msg(user=user['call_info'].get('call_sor_id', ''),
					                receipt='bot', msg=user_input, asr_record_path=asr_record_path)
		...
		bot_say_something = resp_body['outaction'] == '9' and resp_body['outparams']['model_type'].startswith('1') and resp_body['outparams']['prompt_type'] == '2'
		retransmission = self.req_body['inaction'] == 9 and self.req_body['inparams']['flow_result_type'] == '3' and self.req_body['inparams'][
				'input'] == 'timeout'
	...
	def save_results(self, user):
		...
		req_body = [req_body]
		resp = requests.post(self.bot_conf['result_save_url'], data=json.dumps(req_body, ensure_ascii=False, separators=(',', ':')).encode('utf-8'))
